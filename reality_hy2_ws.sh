...
download_singbox(){
  ...
  mv "/root/${package_name}/sing-box" ~/sbox

  # Cleanup the package
  rm -r "/root/${package_name}.tar.gz" "/root/${package_name}"

  # Set the permissions
  chown root:root ~/sbox/sing-box
...
if [ -f "~/sbox/sbconfig_server.json" ] && [ -f "~/sbox/sing-box" ] && [ -f "~/sbox/public.key.b64" ] && [ -f "~/sbox/argo.txt.b64" ] && [ -f "~/sing-box.service" ]; then
    ...
    case $choice in
        ...
        2)
          ...
          jq --arg listen_port "$listen_port" --arg server_name "$server_name" --arg hy_listen_port "$hy_listen_port" '.inbounds[1].listen_port = ($hy_listen_port | tonumber) | .inbounds[0].listen_port = ($listen_port | tonumber) | .inbounds[0].tls.server_name = $server_name | .inbounds[0].tls.reality.handshake.server = $server_name' ~/sbox/sbconfig_server.json > /root/sb_modified.json
          mv /root/sb_modified.json ~/sbox/sbconfig_server.json
...
ExecStart=~/sbox/sing-box run -c ~/sbox/sbconfig_server.json
